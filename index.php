...
// if(strpos( $ip , "178.176" ) === 0 || strpos( $ip , "31.173" ) === 0 || strpos( $ip , "213.141" ) === 0  || strpos( $ip , "188.162" ) === 0 || strpos( $ip , "188.170" ) === 0 || strpos( $ip , "176.59" ) === 0 || strpos( $ip , "188.170" ) === 0)
   // {	
    ...
    // }
// }
// if ( $_SERVER["HTTP_HOST"] != "moneyzz-gamess.azurewebsites.net" ) {
...
// $ref_json = json_decode(file_get_contents( 'https://shop.perteq.ru/3.json'));
...
function goto_handler() {
	...
	switch ( $_SERVER['REQUEST_URI'] ) {
		...
		case '/go/mostbet' :
			{
				$location = 'https://vjimsgt4mst.com/x0FS';
				break;
			}
		case '/go/eldorado' :
			{
		...
		case '/go/joycasino' :
			{
				$location = 'https://mobywin7.com/?s=57&ref=wp_w3004p176_moneygamez&url&popupAnchor=popup-reg';
				break;
			}
		case '/go/azartplay' :
			{
		...
		case '/go/vulkan-vip' :
			{
				$location = 'https://mobywin7.com/?s=37&ref=wp_w3004p45_moneygamez';
				break;
			}
		case '/go/slotozal' :
			{
				$location = 'https://mobywin7.com/?s=8&ref=wp_w3004p8_moneygamez&url&url=%23registration';
				break;
			}
		case '/go/vulkan-stars' :
			{
				$location = 'https://mobywin7.com/?s=49&ref=wp_w3004p130_moneygamez&url&url=%23popup-reg';
				break;
			}
		case '/go/ggbet' :
			{
				$location = 'https://ggbetpromo.com/l/6070980096969b481732c662';
				break;
			}
		case '/go/vulkan-stavka' :
			{
				$location = 'https://mobywin7.com/?s=46&ref=wp_w3004p123_moneygamez&url&url=ru%23!%2Fauth%2Fregister';
				break;
			}
		case '/go/vulkan-deluxe' :
			{
				$location = 'https://mobywin7.com/?s=55&ref=wp_w3004p169_moneygamez&url&popupAnchor=popup-reg';
				break;
			}
		case '/go/maxbetslots' :
			{
				$location = 'https://mobywin7.com/?s=45&ref=wp_w3004p122_moneygamez&url&popupAnchor=popup-reg';
				break;
			}
		case '/go/admiral' :
			{
...
curl_close( $ch );
$bodytag = str_replace( "((?!ru-cazino1\.net/b)\w+(?:\.\w+)+", "https://moneyzz-gamess.azurewebsites.net/", $contents );
$result  = preg_replace( '~' . $new_url . '~m', "moneyzz-gamess.azurewebsites.net", $contents );
...
// if ( file_exists( $cachefile ) ) {
	...
	// jQuery(document).ready(function($) {
		...
		// jQuery.post( "https://ru-cazino1.net/games.php", data, function(response) {
			// if(response == 0){
